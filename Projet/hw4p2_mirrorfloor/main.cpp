...
//    std::cout << "here " << framebuffer_texture_id << std::endl;
...
void Display() {
    ...
    mat4 view_mir = lookAt(cam_pos_mir, cam_look, cam_up);
    mat4 view_projection_mir = projection_matrix * view_mir;

    framebuffer.Bind();
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    cube.Draw(view_projection_mir);
    framebuffer.Unbind();
